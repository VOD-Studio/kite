...
- ~~**播放走内嵌 beep 方案**(零依赖单二进制),不做外部 mpv 调用~~ —— 已废弃(2026-07-27):beep/oto 依赖 cgo 与静态编译冲突,CLI 播放移除,改由 TUI 承担
- **TUI 远期再做**,届时直接做完整 TUI 客户端(侧边栏 + 列表 + mini-player),不分小验证阶段(见 Phase D)

总体顺序:**A 输出层 → C 实用功能 → B 配置 → E 工程化 → D TUI**。
(B/E 独立可穿插;C 依赖 A 的输出约定;D 依赖 C 的播放能力——播放已迁 TUI,见 Phase C 更新注。)
...
验收:`kite search --keyword 周杰伦` 出表格;`... | jq` 管道直接拿到 JSON;
`kite song like --id 1 --yes` 无交互完成;非 TTY 写操作无 `--yes` 报退出码 2。

## Phase C — 实用功能(核心用户价值)✅ 已完成(2026-07-20,PRD-0013)

> **2026-07-27 更新**:CLI 播放(`song play` + `internal/cli/player` 包)已移除——
> beep/oto 依赖 cgo,与静态交叉编译冲突;播放功能将整体迁移到 TUI(PRD-0016)。
> 下方播放/歌词相关内容为历史决策记录,CLI 侧已不再适用。下载、歌单批量仍保留。
...
- 文档:`kite <cmd> --help` 已自足,补 `docs/kite.md` 用户手册(安装/登录/常用流)

### 可发现性与补全(新增,详见 CONTEXT.md「kite CLI」段术语)
...
- **召回池(Recall Pool)**:三类来源汇一池——主动(search/detail)+ 隐式(任何 `--id` 成功消费后透明埋点,**基础设施级,A 类 Context 接入新 rpc 时无需关心**)+ 远端(红心/歌单快照,24h TTL)。持久化 `~/.kite/history.jsonl`(append-only JSONL,1000 行上限,三类用 `src` 字段区分)。预热用「磁盘秒级 + 后台异步拉」,goroutine **fire-and-forget**(不等待、tmp+rename 原子写),绝不阻塞主命令/onboarding
