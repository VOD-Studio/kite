...
kite 从接口调试工具起步,77 个命令挤在单个 `cmd/kite/main.go`(~1300 行),
...
2. **命令树:分组子命令**。`kite playlist tracks --id` 取代平铺的
   `kite playlist-tracks --id`。工具未发布,不做旧命令兼容别名。
...
4. **节奏:一次性搬迁**。行为保持不变的纯重构:输出仍为 protojson JSON,
   写操作仍 y/N 确认,会话文件格式与路径(`~/.kite/session.json`)不变。

## 目标结构

```
cmd/kite/main.go            # 仅 cli.Execute()
internal/cli/
├── root.go                     # 装配所有命令组
├── kit/                        # 命令层共享工具包
