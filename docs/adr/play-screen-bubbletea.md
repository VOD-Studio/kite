# kite 播放屏直接重写为 bubbletea 全屏形态(supersede Phase D 小验证步骤)
...
- 封面渲染采用 oh-my-pi 式协议矩阵(kitty → iterm2 → 半块字符画)与 **transmit-once, place-many** 规则;`KITE_IMAGE_PROTOCOL` env 覆盖对齐 `PI_FORCE_IMAGE_PROTOCOL`。
- roadmap Phase D 段已同步标注第一步 superseded;第二步(全屏播放器)仍待单独立项。
- 裸跑 onboarding 行为不变(CONTEXT.md 工具型定位不受本决策影响);`kite tui` 入口仍属第二步。
