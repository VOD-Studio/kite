...
const (
	...
	StatusWarn Status = "warn" // ! 警告(不影响退出码,合法场景如补全未装)
)
...
// 生产实现注入具体依赖(endpoint/cookie);测试用 fake。
