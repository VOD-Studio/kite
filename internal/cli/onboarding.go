...
//
// 裸跑(无参直接敲 kite)时:未登录给登录引导;已登录按粗粒度时段 + 周末推荐
...
func recommendForTime(now time.Time) []recommendation {
	...
	case bucketNoon:
		return []recommendation{{cmd: "kite recommend playlists", desc: "午休,挑个歌单"}}
	case bucketEvening:
...
func renderOnboarding(w io.Writer, loggedIn bool, now time.Time) {
	if !loggedIn {
		fmt.Fprintln(w, "欢迎使用 kite——网易云接口调试与实用工具。")
		fmt.Fprintln(w)
		fmt.Fprintln(w, "下一步:")
