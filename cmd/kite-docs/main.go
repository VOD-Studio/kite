// Package main 生成 kite 全命令 markdown 参考(cobra GenMarkdownTree)。
//
// 产物落 docs/cmd/(每命令一文件),入库即 web(GitHub)可搜索可链接。
// 与手写流程手册(kite-guide.md)组成文档双轨(clig.dev):
//   - 手册讲流程,命令细节指向 --help / 生成参考(不复制,避免第二份真相腐烂)
//   - 生成参考由命令树生成,freshness 守护测试保证永不脱节
//
// 用法:go run cmd/kite-docs/main.go [输出目录](默认 docs/cmd)
package main
