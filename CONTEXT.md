...
| proxy 优先级链 | `--proxy` > config `proxy` > 环境变量(Go 默认 `ProxyFromEnvironment`,`HTTPS_PROXY`/`HTTP_PROXY`)> 直连;显式设置**整体替换**环境变量层(不混合);不做显式禁用哨兵(`env -u` 在同一层解决) | 2026-08-16 | (无需 ADR,git `http.proxy` 同款覆盖语义;详见 PRD-0018) |
| proxy 范围与防坑 | scheme 白名单 http/https/socks5(标准库原生,零第三方),scheme 必须显式;`config set proxy ""` = **清除 key**(空值有真实语义,区别于 download_dir);代理决策单一来源,**API 与音频下载共用同一 proxy transport**(songdl 不经 engine,只改一侧必分裂) | 2026-08-16 | (无需 ADR;机制见 PRD-0018) |
